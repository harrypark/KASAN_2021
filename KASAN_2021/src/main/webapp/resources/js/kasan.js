...
/**
  202106 수정요청사항
  퇴근시간 -> 조기퇴근시간 변경후
  기준근무시간 이전에 퇴근시에만 "조기퇴근시간"에 퇴근시간표시
*/
function earlyOutTm(gt,wtm, ot){
	//console.log(moment(gt).diff(moment(ot),'seconds'));
	var result = '';
	//console.log(moment(ot,'hh:mm').diff(moment(gt,'hh:mm'),'minutes'))
	timeDiff = moment(ot,'hh:mm').diff(moment(gt,'hh:mm'),'minutes')
		if(wtm>0 && wtm>timeDiff ){
			result =  ot;
		}
...
/**
  202106 수정요청사항
  실제근무시간 -> 근무미달시간 변경후
  기준근무시간 대비 미달한 근무시간만 표시(30분등)
*/
 function belowWorkTm(wt,cwt){
	var belowTm = wt - cwt;
	var result = '';
	if( belowTm > 0 ){
		result = '- '+ minToHour(belowTm);
		}
	return result;
	}
