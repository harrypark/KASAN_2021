...
public class StatServiceImpl implements StatService {
	...
	public List<DailyStat> manualCreateDailyStat(SearchParam searchParam) {
		...
		if(list.size()>0){
			...
			//야근신청 데이터 update - 야근신청결과 승인 업데이트 2021-07-16 까지만 사용.
			// 야근자동신청으로 변경
		...
		logger.debug("#######################################"+(overtimeRuleChange==true?"변경된 야근수당 로직적용":"변경전 야근수당 로직"));

		return statList;
	}

	/** 야근수당 자동 입력 로직 2021-07-09 추가
	 * @param calDt
	 * @param ds
	 */
